...
Set `NOTIFY_ALLOW_PRIVATE_HOSTS=true` to reach an internal receiver. It also permits plain http, since internal endpoints rarely carry certificates. The destination list reports the setting as `allow_private_hosts`, and the settings form relaxes its own address check to match, so the flag takes effect without a frontend change. Enable it only when every account holder on the deployment is trusted to choose an outbound address: it allows anyone who can sign in to make the server issue requests inside your network.
