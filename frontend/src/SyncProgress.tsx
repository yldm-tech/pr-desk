...
const progressSchema = z.object({
  ...
  completed: z.number(), total: z.number(), retry_at: z.number(),
});
export function SyncProgress({connected, pending, onRunningChange}: {
  connected: boolean; pending: boolean; onRunningChange: (value: boolean) => void;
}) {
  ...
  if(connected && query.isError && !pending && !running && !failed) return <div className="sync-status-error" role="status"><span>{t("syncNetworkError")}</span><button className="access-recheck" onClick={()=>query.refetch()}>{t("retry")}</button></div>;
  ...
  return <section className="sync-progress-panel" data-paused={failed || phase === "waiting" ? "true" : "false"} aria-label={t("syncProgress")}>
    <div className="sync-progress-heading"><strong>{t(data?.mode === "incremental" ? "incrementalSync" : "syncProgress")}</strong><span>{phaseLabel}</span></div>
    <ol className="sync-steps" aria-label={t("syncProgress")}>
      {steps.map((key,index)=><li key={key} data-state={index < stage ? "complete" : index === stage ? "active" : "pending"} aria-current={index === stage ? "step" : undefined}>
        <span>{index < stage ? "✓" : index + 1} · {t(key)}</span>
        <motion.div className="sync-step-line" animate={{opacity:index === stage && !failed && phase !== "waiting" && !reducedMotion ? [.45,1,.45] : 1}} transition={{duration:1.5,repeat:Infinity}} />
      </li>)}
    </ol>
    {data?.history_count !== undefined && <p className="sync-collected">{t("syncCollected",{count:data.history_count})}{data.open_count !== undefined ? " · " + t("syncOpenSummary",{count:data.open_count}) : ""}</p>}
