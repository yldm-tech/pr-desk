...
const labels: Record<string, string> = {
  pending: 'Awaiting review', review_requested: 'Review requested',
...
const repositorySchema = z.object({
  repo: z.string(), total: z.number().int().nonnegative(), open: z.number().int().nonnegative(),
  conflicts: z.number().int().nonnegative(), needs_attention: z.number().int().nonnegative(),
});
...
export function parseRepositoryList(body: unknown): RepositorySummary[] {
  return z.object({data:z.array(repositorySchema)}).parse(body).data.map(item => ({
    ...item,
    repo: item.repo.replace(/^https:\/\/api\.github\.com\/repos\//, '').replace(/\/$/, ''),
  }));
}
