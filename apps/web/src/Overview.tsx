...
const schema = z.object({
  visibility_counts: z.object({
    public_repositories: z.number().optional(),
    private_repositories: z.number().optional(),
    unknown_repositories: z.number().optional(),
    public: z.number(),
    private: z.number(),
    unknown: z.number(),
  }),
  ...
  repositories: z.array(
    z.object({ repo: z.string(), total: z.number(), merged: z.number() }),
  ),
...
export default function Overview({
  ...
  const changeScope = (key: string, value: string) => setParams(current => {
    ...
    queryFn: ({ signal }) =>
      ky
        ...
        .then((value) =>
          z
            .object({
              has_installations: z.boolean(),
              can_read_private: z.boolean().optional(),
  ...
  useEffect(() => {
    ...
    const current = JSON.stringify({
      readable: access.data.can_read_private,
      installations: (access.data.installations ?? [])
        .map(({ account, repository_selection, can_read_prs }) => ({ account, repository_selection, can_read_prs }))
  ...
  if (query.isError && !query.data)
    return (
      <div className="empty-state" role="alert">
        <AlertTriangle size={28}/><h2>{t("overviewError")}</h2>
  ...
  const repositoryCountsReady =
    counts.public_repositories !== undefined &&
    counts.private_repositories !== undefined &&
    counts.unknown_repositories !== undefined;
  return (
    <Tabs.Root
      value={String(year)}
      onValueChange={(value) => changeScope("year", value)}
    >
      {query.isError && query.data && <div className="sync-status-error" role="status"><span>{t("refreshFailedKeepData")}</span><button className="access-recheck" onClick={()=>query.refetch()}>{t("retry")}</button></div>}
      <Tabs.Root value={visibility} onValueChange={value => changeScope("visibility", value)}>
        <Tabs.List
          className="visibility-tabs"
          aria-label={t("visibilityScope")}
        >
          <Tabs.Trigger value="public">
            ...
            <span>
              {query.isPlaceholderData
                ? "—"
                : (counts.public_repositories?.toLocaleString() ?? "—")}
            </span>
          </Tabs.Trigger>
          <Tabs.Trigger value="private">
            {t("privateOnly")}{" "}
            <span>
              {access.data?.can_read_private === false ||
              access.data?.has_installations === false
                ? t("notAuthorized")
                : query.isPlaceholderData
                  ? "—"
                  : (counts.private_repositories?.toLocaleString() ?? "—")}
            </span>
          </Tabs.Trigger>
        </Tabs.List>
      </Tabs.Root>
      <p className="visibility-feedback" role="status" aria-live="polite">
        {visibility === "public" ? (
          query.isPlaceholderData ? t("loading") : t("showingPublic", {public:counts.public_repositories ?? 0})
        ) : access.data?.can_read_private === false ||
        access.data?.has_installations === false ? (
          <>
            {t(
              access.data?.has_installations
                ? "privatePermissionMissing"
                : "privateAccessMissing",
            )}{" "}
            <a
              href={
                access.data?.install_url ||
          ...
          <span aria-label={t("checkingAccess")}><Skeleton width={180} height={12}/></span>
        ) : access.isError ? (
          <>
            {t("accessCheckFailed")}{" "}
          ...
          <>
            {repositoryCountsReady
              ? t("showingPrivate", {
                  public: counts.public_repositories,
                  private: counts.private_repositories,
                })
              : t("privateOnly")}
            {visibility === "private" && query.data.visibility_counts.private === 0 &&
              query.data.visibility_counts.unknown === 0 && (
                <> {t("noSyncedPrivate")}</>
              )}
          </>
        )}
        ...
        <div className="authorized-accounts">
          {access.data.installations.map((account) => (
            ...
            >
              {account.account} ·{" "}
              {t(
                account.can_read_prs
                  ? account.repository_selection === "all"
                    ? "allRepositoriesAuthorized"
                    : "selectedRepositoriesAuthorized"
                  : "privatePermissionMissing",
              )}
            </a>
          ))}
      ...
      <Tabs.Content
        value={String(year)}
        className="overview-year-panel"
        aria-busy={query.isFetching}
      >
        {query.isPlaceholderData ? (
          <OverviewSkeleton />
        ) : (
          <Achievements data={query.data} visibility={visibility} />
        )}
      </Tabs.Content>
    </Tabs.Root>
  );
...
function Achievements({ data, visibility }: { data: Data; visibility: string }) {
  ...
  const rate = s.total
    ? new Intl.NumberFormat(i18n.resolvedLanguage, {
      ...
      }).format(s.merged / s.total)
    : "—";
  const percentage = (total: number) =>
    new Intl.NumberFormat(i18n.resolvedLanguage, {
      style: "percent",
      maximumFractionDigits: 2,
    }).format(s.total ? total / s.total : 0);
  const [params, setParams] = useSearchParams();
  const selectedRepo = params.get("repo") || "all";
  const setSelectedRepo = (value: string) => setParams(current => {
    const next = new URLSearchParams(current);
    if(value === "all") next.delete("repo"); else next.set("repo", value);
    return next;
  });
      ...
      defineChart({
        ...
        scales: {
          ...
          y: {
            ...
            nice: true,
            grid: true,
            axis: { line: false, ticks: { size: 0, count: 4, padding: 10 } },
          },
        },
        tooltip: {
          use: tooltip,
  ...
  const distribution = useMemo(() => {
    ...
    const slices = data.repositories.slice(0, 5).map((repo, index) => ({
      label: repo.repo,
      href: `https://github.com/${repo.repo}`,
      value: repo.total,
      share: percentage(repo.total),
      color: colors[index],
    }));
    const other = s.total - slices.reduce((sum, item) => sum + item.value, 0);
    if (other > 0)
      slices.push({
        label: t("otherRepositories"),
        href: "",
        value: other,
        share: percentage(other),
      ...
      defineChart({
        marks: [
          polar({
            ...
            marks: [
              radialArc(
                pie(distribution, { value: "value", gapAngle: 0.025 }),
                {
    ...
    () => [
      {
        ...
        value: s.merged,
        color: "var(--accent)",
        share: percentage(s.merged),
      },
      {
        label: t("openCount"),
        value: s.open,
        color: "var(--warning)",
        share: percentage(s.open),
      },
      {
        label: t("closed"),
        value: s.closed,
        color: "var(--chart-other)",
        share: percentage(s.closed),
      },
    ],
      ...
      defineChart({
        marks: [
          polar({
            inset: 3,
            marks: [
              radialArc(pie(states, { value: "value", gapAngle: 0.035 }), {
    ...
    <section className="overview-page @container/overview" aria-label={t("achievements")}>
            ...
            <div className="outcomes-ring">
              {s.total > 0 && (
                <Chart
                  ...
                  height={166}
                  ariaLabel={t("contributionStates")}
                />
              )}
      ...
      {s.total === 0 ? (
        <p className="achievement-empty">
          {t(data.history_complete ? "noAchievements" : "historySyncNeeded")}
        </p>
      ) : (
        <div className="achievement-bottom grid grid-cols-1 @[800px]/overview:grid-cols-2">
          <article className="achievement-panel trend-panel @container/chart" aria-busy={trendLoading}>
            <div className="trend-header flex-col items-stretch gap-3.5 @[560px]/chart:flex-row @[560px]/chart:items-center">
              <div className="panel-heading"><div><h2>{t("mergeActivity")}</h2><p>{data.months[0]?.month} — {data.months[data.months.length - 1]?.month} · UTC</p></div></div>
              <Select.Root value={repo} onValueChange={setSelectedRepo}>
                <Select.Trigger className="trend-repo-trigger w-full max-w-full @[560px]/chart:w-80 @[560px]/chart:max-w-[48%]" aria-label={t("trendRepository")}><FolderGit2 size={16}/><Select.Value/><Select.Icon><ChevronDown size={14}/></Select.Icon></Select.Trigger>
                <Select.Portal><Select.Content className="language-menu trend-repo-menu" position="popper" align="end" sideOffset={8} collisionPadding={12}><Select.Viewport>
                  <Select.Item className="language-option" value="all"><Select.ItemText>{t("allTrendRepositories")}</Select.ItemText><Select.ItemIndicator><Check size={15}/></Select.ItemIndicator></Select.Item>
                  {data.repositories.map(item=><Select.Item className="language-option" value={item.repo} key={item.repo}><Select.ItemText>{item.repo}</Select.ItemText><Select.ItemIndicator><Check size={15}/></Select.ItemIndicator></Select.Item>)}
                </Select.Viewport></Select.Content></Select.Portal>
              </Select.Root>
            </div>
            <div className="trend-summary-row flex-wrap gap-2"><div className="trend-total"><strong>{trendLoading ? <Skeleton width={90}/> : number(periodTotal)}</strong><span>{t("periodMerged")}</span></div>{repo !== "all" && <a className="trend-repository-link" href={`https://github.com/${repo}`} target="_blank" rel="noopener noreferrer">{repo}</a>}</div>
            {trendLoading ? <div className="trend-skeleton" role="status" aria-label={t("loading")}><Skeleton height={260}/></div> : trendError ? <div role="alert" className="trend-empty"><p>{t("overviewError")}</p><button className="access-recheck" onClick={()=>trendQuery.refetch()}>{t("retry")}</button></div> : periodTotal === 0 ? <div className="trend-empty">{t("noTrendMerges")}</div> : <Chart definition={monthChart} height={280} ariaLabel={t("mergeActivity")}/>}
          </article>
          <article className="achievement-panel distribution-panel block">
                ...
                <TooltipChart
                  ...
                  onSelect={(point) => {
                    if (point?.datum.href)
                      ...
                      document.getElementById("repository-breakdown")?.focus();
                  }}
                ...
                {distribution.map((item) => (
                  <li key={item.label}>
                    ...
                    {item.href ? (
                      ...
                      <button
                        className="other-repositories"
                        onClick={() =>
                          document
                            .getElementById("repository-breakdown")
                            ?.focus()
                        }
                      >
                        {item.label}
            ...
            <div
              ...
              tabIndex={0}
              role="region"
              aria-label={t("repositoryBreakdown")}
            >
              <table>
                <thead>
                  <tr>
      ...
      <p className="achievement-footnote">
        <Info size={14} />
        <span>
          {t(data.history_complete ? "historyScope" : "historySyncNeeded")}{" "}
          {t("yearBasis")}
        </span>
      </p>
    </section>
