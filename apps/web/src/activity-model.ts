...
const comment = z.object({
  id: z.number(), body: z.string(), html_url: z.string(), path: z.string().optional(),
  line: z.number().nullable().optional(), created_at: z.string(),
  user: z.object({login:z.string()}).nullable(),
});
export const activitySchema = z.object({
  conversation: z.array(comment).nullable(), review_comments: z.array(comment).nullable(),
...
export function safeGitHubLink(raw:string):string|undefined {
  try {const url=new URL(raw);return url.protocol==='https:' && url.hostname==='github.com' && !url.username && !url.password ? url.href : undefined;} catch {return undefined;}
}
