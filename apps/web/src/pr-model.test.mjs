...
test('merged and closed states override an old approval', () => {
  const [merged,closed] = parsePRList({data:[{...row,merged_at:'2026-01-01T00:00:00Z',review_status:'approved'},{...row,state:'closed',review_status:'approved'}],total:2});
  assert.equal(merged.status,'Merged'); assert.equal(closed.status,'Closed');
});
test('database ID stays distinct from GitHub PR number', () => {
  const [pr]=parsePRList({data:[row],total:1});
  assert.equal(pr.id,9); assert.equal(pr.number,81); assert.equal(pr.repo,'org/repo');
});
...
test('unknown dates and absent counts have explicit fallbacks', () => {
  ...
  assert.equal(pr.updated,'Unknown');assert.equal(pr.comments,0);assert.equal(pr.conflict,false);
});
