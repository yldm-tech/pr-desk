...
export function UserMenu({ connected, username, onDisconnect, disconnecting, disconnectError }: { connected: boolean; username?: string; onDisconnect: () => void; disconnecting: boolean; disconnectError: boolean }) {
  ...
  return (
    <Popover.Root open={open} onOpenChange={setOpen}>
      <Popover.Trigger className="account-trigger" aria-label={t("personalProfile")}>
        <Avatar login={login} />
        <span>{user?.name || login}</span>
        <ChevronDown size={14} />
      </Popover.Trigger>
      <Popover.Portal>
        <Popover.Content className="profile-popover" align="end" sideOffset={10} collisionPadding={16} aria-label={t("personalProfile")}>
          ...
          {profile.isPending ? (
            ...
            <div role="alert">
              <p>{t("profileError")}</p>
              <button className="profile-retry" onClick={() => profile.refetch()}>
                {t("retry")}
              </button>
            </div>
          ) : (
            user && (
              <>
                <a className="profile-identity" href={href} target="_blank" rel="noopener noreferrer">
                  <Avatar login={login} />
                  <span>
                ...
                <a className="profile-github" href={href} target="_blank" rel="noopener noreferrer">
                  {t("viewGitHubProfile")}
                </a>
              </>
            )
          )}
          <button className="profile-disconnect" disabled={disconnecting} onClick={onDisconnect}>
            <LogOut size={15} />
            {t("disconnect")}
          </button>
...
function Avatar({ login }: { login?: string }) {
  ...
  return login && !failed ? (
    <img className="account-avatar" src={`https://github.com/${encodeURIComponent(login)}.png?size=96`} alt="" onError={() => setFailed(true)} />
  ) : (
