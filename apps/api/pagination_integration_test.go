...
func TestPaginationAndAttention(t *testing.T) {
	...
	// A recently merged PR must not occupy a page slot or count toward totals in the DEFAULT view, even if its old review/conflict fields are still set.
	// It must be reachable when asked for by name, though: merged=true used to be answered with a guaranteed-empty query, so nothing the user had merged or closed could be reached from this endpoint at all while the same page advertised a merged count. An explicit state= or merged= now replaces the open-and-unmerged default rather than being intersected with it, which is why the two cases below expect rows where they once expected none.
	merged := time.Now()
	...
	}{{"?limit=50", 55, 50, 55}, {"?limit=50&offset=50", 55, 5, 5}, {"?attention=true", 1, 1, 55}, {"?merged=true", 1, 1, 56}, {"?search=55", 1, 1, 55}, {"?search=55&offset=1", 1, 0, 0}, {"?search=missing", 0, 0, 0}, {"?search=merged", 0, 0, 0}, {"?search=closed", 0, 0, 0}, {"?state=closed", 2, 2, 56}, {"?review_status=approved", 0, 0, 0}, {"?attention=true&search=54", 0, 0, 0}, {"?search=%25", 0, 0, 0}, {"?search=%5F", 0, 0, 0}, {"?search=%2355", 1, 1, 55}, {"?search=55", 1, 1, 55}, {"?sort=oldest", 55, 50, 1}} {
