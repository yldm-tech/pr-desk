...
// The SDK models destructiveHint and openWorldHint as pointers because the spec defaults both to true when they are absent, so a tool that leaves them nil publishes itself as destructive and as reaching an open world. Nothing here calls out to anything, hence the address of a false. trueHint exists because a hint that is true by default still has to be written down: an annotation the reader can see is what keeps the next tool from being copied from the wrong neighbour.
var (
	falseHint = false
	trueHint  = true
)
...
func (s *Server) newMCPServer() *mcp.Server {
	...
	// destructiveHint does not mean "deletes a row", which is how the write tools below were first annotated; the spec's false means the tool only makes additive updates. The line the five write tools are drawn on is whether the write discards a value the environment cannot recreate. `read` does not: unread is derived as Version > ReadVersion, so the next comment on the pull request makes the row unread again. `handled` does — it overwrites WaitingSince with now, which is how long the thing has been waiting and the number every ranking in this product is built on. `snooze` and `unsnooze` do — a reminder date is a decision, and nothing regenerates the one that was replaced or cancelled. `undo` does — it overwrites whatever the present state is with the snapshot. The one-step snapshot is not a defence here: it is consumed by the next undo and overwritten by the next action, so an agent that fans `handled` across a page of rows destroys every clock but the last, which is exactly the mistake a false hint invites.
	// IdempotentHint follows the same reading, and `read` is the only one left holding it. Repeating `handled` or `snooze` writes a fresh `now` and lands somewhere the first call did not; repeating `unsnooze` leaves the row identical but spends the snapshot on a no-op, so the reminder the first call cancelled is no longer recoverable. A second call having no additional effect is the whole claim, and for those three it is false. `read` earns it only because this pass stopped it from snapshotting: there is now nothing for a repeat to consume.
	...
	mcp.AddTool(server, &mcp.Tool{Name: "mark_follow_up_read", Description: "Mark a follow-up as read. This does not mark the work as handled, does not touch GitHub, and is not undoable: new activity on the pull request marks the row unread again by itself, so there is no snapshot to go back to.", Annotations: &mcp.ToolAnnotations{IdempotentHint: true, DestructiveHint: &falseHint, OpenWorldHint: &falseHint}}, s.followUpAction("read"))
	mcp.AddTool(server, &mcp.Tool{Name: "mark_follow_up_handled", Description: "Mark a follow-up as handled. This restarts its waiting clock, which discards how long the pull request had been waiting — the number the workspace ranks by — and only the most recent such action can be taken back, with undo_follow_up. Nothing is posted to GitHub.", Annotations: &mcp.ToolAnnotations{IdempotentHint: false, DestructiveHint: &trueHint, OpenWorldHint: &falseHint}}, s.followUpAction("handled"))
	mcp.AddTool(server, &mcp.Tool{Name: "snooze_follow_up", Description: "Stop reminding about a follow-up for a number of days, replacing any reminder already set on it. Technical failures and new human feedback can still surface it.", Annotations: &mcp.ToolAnnotations{IdempotentHint: false, DestructiveHint: &trueHint, OpenWorldHint: &falseHint}}, s.followUpAction("snooze"))
	mcp.AddTool(server, &mcp.Tool{Name: "unsnooze_follow_up", Description: "Cancel a snooze and let the follow-up surface again, discarding the date it was set to. Read and handled state are left alone.", Annotations: &mcp.ToolAnnotations{IdempotentHint: false, DestructiveHint: &trueHint, OpenWorldHint: &falseHint}}, s.followUpAction("unsnooze"))
	...
	mcp.AddTool(server, &mcp.Tool{Name: "undo_follow_up", Description: "Take back the last handled, snooze or unsnooze on a follow-up, restoring the read, handled, confirmation and waiting state it had before. Marking a follow-up read is not recorded and so is never what this takes back. One step only: the snapshot is consumed, so calling it twice does nothing the second time, and it overwrites the current state, so calling it on a row whose last action you did not make will discard that action.", Annotations: &mcp.ToolAnnotations{IdempotentHint: false, DestructiveHint: &trueHint, OpenWorldHint: &falseHint}}, s.followUpAction("undo"))
	return server
}
